...
まず設定をコピーし、先頭の `work_dir` を編集します。

```bash
cp -n config/example.yaml config/agung.yaml
```
...
`work_dir` はホスト上の作業ルートです。相対パスの場合は **YAML のあるディレクトリ** が基準です。
例えば `config/agung.yaml` に `work_dir: ../work/agung` と書くと、リポジトリの `work/agung` を使います。
`paths` 以下のパスは `work_dir` 内の相対パスです。`paths.dem` は処理に使う DEM ファイル名で、
`init` は DEM ファイル自体を作りません。未設定の `bbox` などは初期化・取得には不要です。

```bash
scripts/run.sh init config/agung.yaml
```

このコマンドは設定の検証、作成先の表示、ディレクトリ作成だけを行います。
親ディレクトリも作るため、先に `mkdir` する必要はありません。
設定した場所にファイルがある場合は、ディレクトリを作り始める前にエラーにします。
既存データは上書き・削除しません。同じ設定で再実行できます。
権限不足や容量不足など、作成中の IO エラーでは途中までディレクトリが残る場合があります。
...
最初の設定 YAML はリポジトリ側に置いたまま使います。作業ルート内の `config/` は
将来の処理設定生成用で、`init` はそこへ YAML をコピーしません。
JSON 専用・アーカイブ用ディレクトリは作りません。

`run.sh` はコンテナで YAML を読み、`init` のディレクトリ作成はホストの現在のユーザーで行います。
`download` では設定した作業ルートをコンテナの `/work` に接続して実行します。
UID/GID は自動取得します。**この起動方法では `.env` を読みません。`WORK_ROOT` の設定は不要です。**
処理内容・保存場所は端末に表示します。使用イメージを変える場合は
`scripts/run.sh --image IMAGE init config/agung.yaml` の形で指定します。
イメージのビルドは自動実行しないので、コード変更後は冒頭の build コマンドを実行してください。

WSL で外部 SSD を使う場合は、初期化前に `findmnt -M /mnt/ssd` で
実際のマウントを確認してください。Docker Desktop から `not a shared mount`
となる環境では `sudo mount --make-shared /mnt/ssd` が必要です。
この属性は再起動後に確認し直します。
...
- 取得開始前（認証後の転送先を含む）の DNS エラーは、1秒・2秒・4秒待って最大3回再試行し、端末とログに表示します。解消しなければ停止します。認証エラーや受信途中の通信エラーは、この自動再試行の対象外です。
- `.part` に取得し、**サイズと MD5 が一致した後にだけ**正式な ZIP 名にします。
- 正常な既存 ZIP は検証してスキップします。完成済みの `.part` は正式名に変更します。
- 未完成の `.part` は再実行時に先頭から取得し直します。HTTP Range による途中再開は未対応です。
...
この出力はラップ位相用です。アンラップ結果のGeoTIFF化とPGVアルゴリズム自体は含みません。
...
CI の設定と同じ検査のローカル確認を行っていますが、GitHub 上での初回実行はまだです。
リポジトリを GitHub に接続し、ソースとこの workflow を push すると実行対象になります。
...
[配布候補の手順](docs/releases/0.1.0-rc1.md) を参照してください。正式公開前の候補です。
