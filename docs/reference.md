...
DEMの取得・検証成功後、`paths.dem` を自動更新します。コメントと他設定を保持し、変更前のYAMLを隣にバックアップします。失敗・dry-run・作業ルート外の出力では更新しません。`--no-update-config` で無効にできます。rc4公開後の追加機能のため、配布済みrc4では手動設定が必要です。
