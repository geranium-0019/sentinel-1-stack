...
DEMの取得・検証成功後、`paths.dem` を自動更新します。コメントと他設定を保持し、変更前のYAMLを隣にバックアップします。失敗・dry-run・作業ルート外の出力では更新しません。`--no-update-config` で無効にできます。rc4更新版（2026-09-22）から対応しています。
...
並列数を変えても完了済み工程を保持して再開できます。アンラップはペアごとに終了コード0の完了記録を保存し、途中で停止しても完了ペアを再利用します。標準SNAPHU出力の保存済みサイズ・更新時刻が変わった場合は再実行します。

旧版の記録でも、1並列で次のペアの開始が記録されていれば、その前のペアは正常終了したと確認できます。コマンド順と標準SNAPHU出力（寸法・サイズ・連結成分内の有限位相）を検証し、確認できたものだけ引き継ぎます。旧版の2並列記録や最後に開始したペアは推測で完了扱いにしません。元のrun.jsonは別名で保存します。

`run --resume --dry-run --unwrap-jobs 2` で再利用数と残りを確認できます。確認だけでは実行記録を変更しません。通常の再開は `run --resume --unwrap-jobs 2` です。生成済みのwait境界は保つため、最大2並列でも1ペアだけ動く区間があります。
