...
この更新版には、bboxからのswath自動選択、基準日の自動選択・設定保存、DEM成功後の設定自動更新、アンラップのペア単位の再開を含みます。同じrc4名で再配布しているため、更新前に取得した方はイメージ・操作用ファイル・SHA256SUMSの3点を取り直してください。既存の操作用フォルダは退避してから展開し、イメージも再度 `docker load` してください。観測データの作業ディレクトリを消す必要はありません。

`image-inspect.json` のrevisionラベルとSHA256SUMSで、使用する内容を識別できます。
...
失敗した場合は公開せず、Actionsに検査ログを残します。原則として新しいバージョンを公開します。今回は利用者の指示によりrc4だけを更新し、検証成功後に全配布ファイルを差し替えます。
